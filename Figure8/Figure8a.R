...
svg(filename="Figure8a_new.svg", 
    width=7, 
    height=3.5, 
    pointsize=12)
par(mfrow=c(1,2))
par(mar=c(4,4,1,1))
par(cex.axis=0.6, cex.lab=0.6, cex.main=0.6, cex.sub=0.6)cex.lab=0.6, cex.main=0.6, cex.sub=0.6)

plot(x_vector,alpha_opt_vector, type="l", lty =1, col = 'red', ylab = 'relative CH4 emission rate change (%)', xlab = 'change in parameter (%)', )
...
legend(-50, 30, legend=c("alpha_opt","qmax","yield","meth enrich","initial and inlet methanogens","ks"), col=c("red","palegreen4","blue","orange","purple",'black'), lty = c(1,1,1), cex = 0.6, box.lty = 0, bg='transparent')
dev.off()